...
import { getTagByReceiptStatus, getTagStatusColor } from '../../utils/date';

const HeaderTitleTagWhIn = ({ receipt, label }) => (
    <>
        <div className="table-toolbar header-tag">
            <h3>{receipt.refCode}</h3>
            <Tag color={getTagStatusColor(receipt.status)} style={{marginTop: '10px', marginLeft: '10px'}}> {getTagByReceiptStatus(receipt.status).toUpperCase() } </Tag>
        </div>
